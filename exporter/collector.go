...
func (mc JSONMetricCollector) Collect(ch chan<- prometheus.Metric) {
	for _, m := range mc.JSONMetrics {
        level.Error(mc.Logger).Log("msg", "metrics ", "path", m.KeyJSONPath, "metric", m.EpochTimestampJSONPath)
		switch m.Type {
...
func timestampMetric(logger log.Logger, m JSONMetric, data []byte, pm prometheus.Metric) prometheus.Metric {
        level.Error(logger).Log("msg", "m.EpochTimestampJSONPath", m.EpochTimestampJSONPath)
...
//	ts, err := extractValue(logger, data, "{.timestamp}", false)
        if ts == "" {
            return pm
        }
	if err != nil {
        ...
        ti, err := time.Parse(layout, ts)
        epoc := ti.Unix()
//	epochTime, err := SanitizeIntValue(string(epoc))
//        epochTime, err := SanitizeIntValue(ti)
	if err != nil {
		level.Error(logger).Log("msg", "Failed to parse timestamp for metric", "path", m.KeyJSONPath, "err", err, "metric", m.Desc, "ts", ts)
		return pm
	}
        timestamp := time.UnixMilli(epoc)
        fmt.Println(timestamp)
        level.Error(logger).Log("msg", "timestamp", timestamp)
